...
\end{enumerate}
Y en estas condiciones, $\norm[L(E,\ F)]{T} = \norm[L(F',\ E')]{T\adj}$ % TODO: Cambiarle a la L bonita
\end{thm}
...
Escribamos $y_n = T f_n$ con $f_n \in D(T\adj)$
...
\left.
\begin{array}{r}
     f_n \in D(T\adj), f_n \to f \text{ en $F'$}\\
     y_n = T\adj f_n \to y  \text{ en $E'$}
\end{array}\right \}
\overset{\text{$T\adj$ es cerrado}}{\implies}  (f,\ y) \in \gr T\adj \text{, ie, } f \in D(T\adj),\ y = T\adj f
$$
